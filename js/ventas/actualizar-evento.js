    ...
    $q("#btnActualizarEvento").addEventListener("click", async () => {
        try {
            ...
            const horarioSuperpuesto = fechaOcupada.some(evento => {
                ...
                if (eventoFinal < eventoInicio) eventoFinal += 1440; // Sumamos 24 horas en minutos
                if (finalNuevo < inicioNuevo) finalNuevo += 1440; 
    
                return evento.fecha_presentacion === fechaSeleccionada &&
                    !(
                        finalNuevo <= eventoInicio || // Nuevo evento termina antes del inicio del existente
                        inicioNuevo >= eventoFinal   // Nuevo evento empieza después de que el existente termina
                    );
            });
