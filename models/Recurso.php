...
class Recurso extends ExecQuery
{
  ...
  public function obtenerConceptos(): array
  {
    try {
      $cmd = parent::execQ("SELECT * FROM conceptos");
      $cmd->execute();
      return $cmd->fetchAll(PDO::FETCH_ASSOC);
    } catch (Exception $e) {
      die($e->getMessage());
  ...
  public function obtenerSubTipoPorIdConcepto($params = []): array
  {
    try {
      $cmd = parent::execQ("SELECT * FROM subtipos where idconcepto = ?");
      $cmd->execute(array($params['idconcepto']));
      return $cmd->fetchAll(PDO::FETCH_ASSOC);
    } catch (Exception $e) {
      die($e->getMessage());
    }
